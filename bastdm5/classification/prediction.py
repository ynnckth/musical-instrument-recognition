...
from bastdm5.classification.test_batchiterator import TestSegmentBatchIterator
from bastdm5.classification.utils.spectrogram_tools import spectrogram_utils

import bastdm5.classification.settings
from bastdm5.classification.utils import utils

CNN_WEIGHTS_PATH = os.path.normpath(os.path.join(os.getcwd(), "bastdm5/classification/trained_models/full_set_model_5000/23-15_1-6-2016_net_weights.pickle"))
CNN_Y_MAPPING_PATH = os.path.normpath(os.path.join(os.getcwd(), "bastdm5/classification/trained_models/full_set_model_5000/23-15_1-6-2016_y_mapping.pickle"))
...
def get_spectrograms_from_filepath(audio_file_path):
    ...
    segments = segment_signal(signal, sampling_rate, bastdm5.classification.settings.SEGMENT_SECONDS)
...
def filter_zero_segments(instrument_segments):
    SEGMENT_MAX_ZERO_THRESHOLD = int(
        bastdm5.classification.settings.SEGMENT_LENGTH * bastdm5.classification.settings.MEL_DATA_POINTS * 0.9)
    clean_instrument_segments = []
    segment_idx = 0
    for segment in instrument_segments:
        zeros_in_segment = bastdm5.classification.settings.SEGMENT_LENGTH * bastdm5.classification.settings.MEL_DATA_POINTS - np.count_nonzero(segment[0])
...
def init_cnn():
    net = NeuralNet(
        layers=[
            # input layer
            (layers.InputLayer,
             {'shape': (None, bastdm5.classification.settings.CHANNELS, bastdm5.classification.settings.MINI_SEGMENT_LENGTH, bastdm5.classification.settings.MEL_DATA_POINTS)}),
    ...
    net.batch_iterator_test = TestSegmentBatchIterator(batch_size=bastdm5.classification.settings.MINI_BATCH_SIZE)
